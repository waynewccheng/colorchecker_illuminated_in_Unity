...
%% location
%
% ==== modify here to change the pixels of interest ====
%
xy = [513 87;
    687 204;
    381 234;
...
%% deliverable
data = zeros(7,size(xy,1),22);
data_dE = zeros(7,size(xy,1));
...
for group_no = 1:7
    ...
    for i = 1:size(xy,1)
        ...
        data(group_no,i,1:3) = rgb_target;
        data(group_no,i,4:6) = rgb_measure;
        data(group_no,i,7:9) = rgb_measure_wp;
        data(group_no,i,10:12) = xyz_measure;
        data(group_no,i,13:15) = xyz_measure_wp;
        data(group_no,i,16:18) = lab_target;
        data(group_no,i,19:21) = lab_measure;
        data(group_no,i,22) = dE;
        
        data_dE(group_no,i) = dE;
    end
...
% save the data
%
save(sprintf('%s\\results',pathnameoutput),'data','data_dE') % in Matlab

xlswrite(sprintf('%s\\dE.xls',pathnameoutput),data_dE)           % in Excel
